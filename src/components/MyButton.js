...
export default function MyButton({ label, primary = true, white, flat, dark, disabled, loader }) {
	console.log("🚀 ~ file: Button.jsx:6 ~ Button ~ { label, primary, white, flat, dark }:", {
		label,
		primary,
		white,
		flat,
	...
	if (flat) {
		if (white) {
			classes.push("button--flat-white");
		} else {
			classes.push("button--flat");
		}
	} else if (primary) {
		if (white) {
			classes.push("button--primary-white");
		} else {
			classes.push("button--primary");
		}
	} else {
		if (white) {
			classes.push("button--secondary-white");
		} else if (dark) {
			classes.push("button--secondary-dark");
		} else {
	...
	return (
		<>
			{!loader ? (
				<button disabled={disabled} className={classes.join(" ")}>
					{label}
				</button>
			) : (
				<button disabled={disabled} className={classes.join(" ")}>
					<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
						<g clip-path="url(#clip0_2659_1351)">
							<path
